...
function removeFirstOccurrences(str, value) {
  // const arr = str.split(' ');
  // Looks like mistake, it needs to use indexOf but its completely useless here. i've just put indexOf in comments
...
function removeLastOccurrences(str, value) {
  ...
  // console.log(str.slice(lastIndex + value.length));
  if (lastIndex === -1) {
    return str;
  }
  const replaced =
    str.slice(0, lastIndex) + replacement + str.slice(lastIndex + value.length);
...
function isPalindrome(str) {
  ...
  // console.log(newstr);
  return (
    newstr.toLowerCase() === newstr.toLowerCase().split('').reverse().join('')
  );
}
...
function extractEmails(str) {
  return str.split(';').filter((elem) => elem.includes('@'));
}
